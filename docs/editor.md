...
* `plan`, representing the chosen service plan or plan template. Within this are two further objects:
  * `plan.detail` contains the overall plan properties. When viewing dated plans, it is a
    [Plan model](https://developer.churchsuite.com/planning#models/plan) with one addition, the `date_time`
    property which represents the plan's start date and time as a JavaScript Date object.
    When viewing a plan template, it is a [Template model](https://developer.churchsuite.com/planning#models/Template)
  * `plan.items` is an array of objects, each representing an item in the service plan.
    When viewing dated plans, each item is a [PlanItem model](https://developer.churchsuite.com/planning#models/planitem)
    ...
    When viewing plan templates, each item is a [TemplateItem model](https://developer.churchsuite.com/planning#models/TemplateItem)
  * If **Show song lyrics** is selected, and a service item represents a song, the item will additionally have an `arrangement` property
    which is a [SongArrangement model](https://developer.churchsuite.com/planning#models/songarrangement), and a `song` property which
    is a [Song model](https://developer.churchsuite.com/planning#models/song). The `arrangement` property is in turn augmented with a
...
* `account` represents key details from your church's account as an [Account Info model](https://developer.churchsuite.com/account#models/Info).
  minus the `billing_address`, `billing_contact`, `data_protection_contact` and `account_contact properties`
* `brand` represents your account's default branding as a [Brand model](https://developer.churchsuite.com/account#models/brand),
...
* `types` contains a set of properties each representing one of your plan item types as
  [Type model](https://developer.churchsuite.com/planning#models/type), and with the key being the type name converted
...
to reference your ChurchSuite account's primary colour, and use `contrast-color(var(--primary-color))` to provide a constrast
with it.
